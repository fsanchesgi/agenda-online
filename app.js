...
async function gerarCheckout(planoNome, preco, perfilId) {
  try {
    ...
    if (data.init_point) {
      ...
      alert("Erro ao gerar checkout"); console.error(data);
    }
  } catch (err) { console.error(err); alert("Erro no processo de pagamento"); }
}
...
async function carregarProfissionais() {
  const { data, error } = await supabaseClient.from("profissionais").select("id, nome").eq("ativo", true);
...
async function carregarServicos() {
  const { data, error } = await supabaseClient.from("servicos").select("id, nome, duracao_minutos");
  if (error) return console.error(error);
...
async function carregarHorarios() {
  ...
  const { data, error } = await supabaseClient.rpc("horarios_disponiveis", { p_profissional_id: profissionalSelect.value, p_data: dataInput.value, p_duracao_minutos: duracao });
  if (error) return console.error(error);
...
async function agendar() {
  ...
  const { data: podeAgendar, error: erroLimite } = await supabaseClient.rpc("pode_criar_agendamento", { p_profissional_id: profissionalSelect.value });
  if (erroLimite) { alert("Erro ao validar plano"); return; }
  if (!podeAgendar) { alert("Limite do plano gratuito atingido.\nFaça upgrade para continuar."); return; }

  const dataHora = `${dataInput.value} ${horarioSelect.value}`;
  const { error } = await supabaseClient.from("agendamentos").insert([{ profissional_id: profissionalSelect.value, servico_id: servicoSelect.value, cliente_nome: clienteNome, data_hora: dataHora, status: "agendado" }]);
